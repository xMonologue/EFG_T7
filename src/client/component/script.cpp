...
namespace script
{
	namespace
	{
		...
		void load_script(std::string& name, const std::string& data, const bool is_custom)
		{
			...
			const auto host_path = (utils::nt::library{}.get_folder() / "t7efg/").generic_string();
		...
		void load_scripts()
		{
			...
			const auto t7efg_folder = host.get_folder() / "t7efg";

			const auto load = [&data_folder, &t7efg_folder](const std::filesystem::path& folder, const bool is_custom)
			{
				load_scripts_folder((data_folder / folder).string(), is_custom);
				load_scripts_folder((t7efg_folder / folder).string(), is_custom);
			};
